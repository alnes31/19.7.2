...
class PetFriends:
    ...
    def add_new_simple_pet(self, auth_key: json, name: str, animal_type: str,
                    age: str) -> json:
        """Метод отправляет (постит) на сервер данные о добавляемом питомце и возвращает статус
        запроса на сервер и результат в формате JSON с данными добавленного питомца"""
        ...
        headers = {'auth_key': auth_key['key'], 'Content-Type': 'application/json'}

        res = requests.post(self.base_url + '/api/create_pet_simple', headers=headers, data=json.dumps(data))
        status = res.status_code
        result = ""
        try:
            result = res.json()
